...
const DB_NAME = 'belize_rdt_v110'; // Nova versão para resetar e validar testes
...
const holidays = {
    ...
    "04-21": "Tiradentes", "04-22": "Aniversário Itanhaém",
    "05-01": "Dia do Trabalho", "06-04": "Corpus Christi",
    "07-09": "Revolução Constitucionalista",
    "09-07": "Independência", "10-12": "Nossa Sra. Aparecida",
    "11-02": "Finados", "11-15": "Proclamação República",
...
// --- EXPEDIENTE AJUSTADO PARA TESTE ---
function isWorkHours() {
    ...
    if (dia >= 1 && dia <= 5) return hora >= 8 && hora < 17; // Seg a Sex
    if (dia === 6) return hora >= 8 && hora < 23; // SÁBADO AMPLIADO PARA TESTE (ATÉ 23H)
    return false;
}
...
function showRestMessage() {
    ...
    document.getElementById('rest-message').innerText = "O descanso é essencial. Aproveite sua família e recarregue as energias!";
}
...
function renderAdminUI() {
    ...
    if(adm.length) {
        ...
        accordion.innerHTML += `<div class="worker-card"><div class="worker-header" style="background:var(--belize);color:white">Minhas Atividades</div><div class="worker-content">${h}</div></div>`;
    }
...
function renderWorkerUI() {
    ...
    // CORREÇÃO: Pega a data de hoje no formato do sistema (YYYY-MM-DD)
    const hojeSistema = new Date().toLocaleDateString('en-CA');
    
    const tasks = db.team[u].tasks.filter(t => t.date === hojeSistema);
    const list = document.getElementById('worker-task-list');
    
    list.innerHTML = tasks.length > 0 
        ? tasks.map(t => `<div class="card"><b>${t.desc}</b><br><small>Status: ${t.status}</small></div>`).join('')
        : `<p style="text-align:center; opacity:0.6">Nenhuma tarefa encontrada para hoje (${new Date().toLocaleDateString('pt-BR')})</p>`;
}
